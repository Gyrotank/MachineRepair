			...
			<div class="content" id="serviceable_machines">
					...
					<thead>
						<tr>
							...
							<th align="center"><spring:message
									code="label.adminpage.serviceableMachines.name" /></th>
							<th align="center"><spring:message
									code="label.adminpage.serviceableMachines.trademark" /></th>
							<th align="center"><spring:message
									code="label.adminpage.serviceableMachines.country" /></th>
							<th align="center" data-sortable="false"><spring:message
									code="label.adminpage.serviceableMachines.actions" /></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="ms" items="${entities}"
							...
							<tr class="${loopStatus.index % 2 == 0 ? 'even' : 'odd'}">
								...
								<td><c:choose>
										<c:when test="${ms.available == 0}">
											<div class="disabled">${ms.machineServiceableName}</div>
										</c:when>
										<c:otherwise>
    			${ms.machineServiceableName}
								...
								<td><c:choose>
										<c:when test="${ms.available == 0}">
											<div class="disabled">
												${ms.machineServiceableTrademark}</div>
										</c:when>
										<c:otherwise>
								...
								<td><c:choose>
										<c:when test="${locale == 'ru'}">
											<c:choose>
												<c:when test="${ms.available == 0}">
													<div class="disabled">
														${ms.machineServiceableCountryRu}</div>
												</c:when>
												<c:otherwise>
    					${ms.machineServiceableCountryRu}
										...
										<c:otherwise>
											<c:choose>
												<c:when test="${ms.available == 0}">
													<div class="disabled">
														${ms.machineServiceableCountry}</div>
												</c:when>
												<c:otherwise>
				...
				<br id="add_new_serviceable_machine">
				<div>					
					<h2>
						...
						<table>
							<tr>
								...
								<td><form:input path="machineServiceableName"
										id="machineServiceableNameInput" maxlength="50" /></td>
							...
							<tr>
								...
								<td><form:input path="machineServiceableTrademark"
										id="machineServiceableTrademarkInput" maxlength="50" /></td>
							...
							<tr>
								...
								<td><form:input path="machineServiceableCountry"
										id="machineServiceableCountryInput" maxlength="50" /></td>
							...
							<tr>
								...
								<td><form:input path="machineServiceableCountryRu"
										id="machineServiceableCountryRuInput" maxlength="50" /></td>
